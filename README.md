...
- Changed lines go to TypeSafe under your key, or to an endpoint you set yourself, and nowhere else. No server of ours is in the path.
...
- To send the direct call somewhere other than typesafe.ai (a self-hosted deployment, a local proxy, a mock), set `TYPESAFE_AI_BASE_URL` alongside your TypeSafe key. It is read from the same places as the key, never written or logged, and applies only to the direct call, not the gateway. The key goes to that URL.
- For an endpoint that needs no key, such as a local [Laya](https://huggingface.co/convaiinnovations/laya), set `ABIDE_ENDPOINT_URL` instead. With `laya-serve` on its default port, that is `ABIDE_ENDPOINT_URL=http://127.0.0.1:8000/v1`. It wins over any key you have saved, sends no `Authorization` header and counts each check as free. It is read from the environment or `~/.abide/.env` only, never from a file in the repo, so a repo you clone cannot choose where your diffs go. If you start `laya-serve` with `LAYA_API_KEY`, use `TYPESAFE_AI_BASE_URL` and a `TYPESAFE_AI_API_KEY` with the same value instead. The bands and the numbers below were measured against Jev, so run `abide calibrate` before you trust another model's verdicts.
