...
// the abide hook script already understands, runs that script, and puts the
      ...
      try {
        ...
        if (!s) {
          ...
          if (typeof context === "string" && context !== "") {
            output.parts.push({
              id: `prt_abide_${Date.now().toString(36)}`,
              sessionID,
        ...
        // Abide's own repair request continues the turn it was raised in.
        if (s.repairing && text.startsWith("Abide:")) {
          s.repairing = false;
          return;
        }
        s.turnId = message.id;
