...
- **No secrets in this repo, ever.** The API key is read from the environment only. Never accept it as a flag — a flag lands in shell history and in CI logs — never write it to disk, and never log it.
